...
Ext.canvasXpress = Ext.extend(Ext.Panel, {
  ...
  contextMenu: true,
  menuTitle: 'Menu',
//  menuTitle: this.languages.menu,TODO
  showPrint: true,
  changedNodes: [],
  changedEdges: [],
  ...
  networkMenu: function(o, type, it) {
    ...
    if(type == 'cs' ) // right click menu only && droit d'écriture
    {
      ...
      if(o && o.nodes && o.nodes.length && (o.nodes[0].testEvent == false)) // canvasXpress only sends 1 node over
      {
        ...
        if(o.nodes.length > 1)
        {
          ...
          items.push(/*{
            ...
            text: this.languages.deleteSelectedObjects,
            scope: this,
          ...
          items.push(
           {
              ...
              text: this.languages.informationsAbout + ' ' + label,
              handler: this.creationfenetre.createDelegate(this,[n])
           },
          {
            icon: this.imgDir + 'edit.png',
            text: this.languages.edit+' ' + label,
            handler: this.editNode.createDelegate(this, [n], true)
          },/* {
            ...
            icon: this.imgDir + 'delete.png',
            text: this.languages.delete1 + ' ' + label,
            scope: this,
            handler: this.deleteNode.createDelegate(this, [n])
          }, {
            icon: this.imgDir + 'cancel.png',
            text: this.languages.hide + ' ' + label,
            handler: this.toggleNode.createDelegate(this, [n, true])
          }/*, {
            text: 'Order of ' + label,
            menu: [
        ...
        if(o && o.nodes && o.nodes.length && (o.nodes[0].testEvent == true)) // canvasXpress only sends 1 node over
        {
            ...
            if(o.nodes.length > 1)
            {
                ...
                items.push(
                    {
                        icon: this.imgDir + 'information.png',
                        text: this.languages.informationsAboutSelectedItems,
                        handler: this.creationfenetre.createDelegate(this,[n])
                    }
                );
                Ext.canvasXpress.utils.addMenu(items, n, this.nodeMenu, this);
            ...
            else
            {
                ...
                items.push(
                    {
                        icon: this.imgDir + 'information.png',
                        text: this.languages.info + ' ' + label,
                        handler: this.creationfenetre.createDelegate(this,[n])
                    }
                );
                Ext.canvasXpress.utils.addMenu(items, n, this.nodeMenu, this);
      ...
      else
      {
        ...
        var hidden = this.canvas.getHiddenNodes(), hn = [{
          ...
          text: this.languages.allHiddenObjects,
        ...
        if(hn.length > 1)
          items.push({
            text: this.languages.seeAllHiddenObjects,
            menu: hn
          });
        if(this.canvas.canNetworkUndoOp())
          items.push({
            text: this.languages.cancelModif,
        ...
        if(this.canvas.canNetworkRedoOp())
          items.push({
            text: this.languages.replayModif,
        ...
        {
          items.push('-', {
            text: this.languages.saveNetwork,
            icon: this.imgDir + 'add.png',
            handler: this.editNetwork.createDelegate(this, [true])
          });
          if(this.networkInfo.id)
            items.push({
              text: this.languages.edit + ' ' + this.languages.network.toLowerCase(),
              icon: this.imgDir + 'edit.png',
              scope: this,
          ...
          if(this.hasUnsavedChanges() && this.hasListener('saveallchanges'))
            items.push({
              text: this.languages.saveNetworkNow,
        ...
        items.push(
          {
            text: this.languages.simpleSearch,
   ...
   creationfenetre: function(n){
       ...
       for(var i = 0; i < n.length; i++){
           ...
           win = new Ext.Window({id:n[i].id,title:this.languages.information +' - '+n[i].name,width:'600px',height:'200px',
  ...
  createContextMenu: function (e) {
    ...
    this.addItemToMenu(items, this.languages.general, false, this.generalMenus());
    this.addItemToMenu(items, this.languages.axes, false, this.axesMenus());
    this.addItemToMenu(items, this.languages.labels, false, this.labelMenus());
    this.addItemToMenu(items, this.languages.legend, false, this.legendMenus());
    this.addItemToMenu(items, this.languages.indicators, false, this.indicatorMenus());
    this.addItemToMenu(items, this.languages.decorations, false, this.decorationMenus());
    this.addItemToMenu(items, this.languages.data, false, this.dataMenus());
    items.push({
      icon: this.imgDir + 'refresh.png',
      text: this.languages.refreshGraph/* + this.canvas.graphType*/,
      scope: this.canvas,
    ...
    {
      items.push('-');
      items.push({text: this.languages.seeAsPicture,
  ...
  axesMenus: function () {
    ...
    var axes = this.canvas.getValidAxes();
    if (axes) {
      this.addItemToMenu(items, this.languages.fontWeight, false, this.axesFontSizes());
      this.addItemToMenu(items, this.languages.color, false, this.axesColors(axes));
      this.addItemToMenu(items, this.languages.scaleFactor, false, this.axesFontScaleFactors());
      this.addItemToMenu(items, this.languages.show, false, this.axesShow(axes));
      this.addItemToMenu(items, this.languages.properties, false, this.axesProperties(axes));
      this.addItemToMenu(items, this.languages.title, false, this.axesTitles(axes));
      this.addItemToMenu(items, this.languages.ticks, false, this.axesTicks(axes));
      this.addItemToMenu(items, this.languages.transform, false, this.axesTransforms(axes));
    }
    return items;
  },
  ...
  generalMenus: function () {
    var items = [];
    this.addItemToMenu(items, this.languages.color, false, this.generalColors());
    //this.addItemToMenu(items, 'Dimensions', false, this.dimensions());
    this.addItemToMenu(items, this.languages.font, false, this.font());
    this.addItemToMenu(items, this.languages.layout, false, this.graphLayout());
    this.addItemToMenu(items, this.languages.title, false, this.titles());
    this.addItemToMenu(items, this.languages.orientation, false, this.graphOrientation());
    this.addItemToMenu(items, this.languages.overlays, false, this.overlays());
  ...
  generalColors: function() {
    return [{
      text: this.languages.background,
      iconCls: '',
  ...
  background: function () {
    var m = [];
    m.push({text: this.languages.type,
	    iconCls: '',
	    menu: this.backgroundType()});
    if (this.canvas.backgroundType == this.languages.gradient) {
      m.push({text: this.languages.colors,
	      iconCls: '',
	      menu: this.backgroundGradients()});
    } else {
      m.push({text: this.languages.color,
	      iconCls: '',
  ...
  backgroundType: function () {
    return this.buildGroupMenu('backgroundType', [this.languages.solid, this.languages.gradient]);
  },
  ...
  titles: function () {
    var items = [{
      text: this.languages.title,
      iconCls: '',
      menu: this.titlesTitle()
    }];
    if (this.canvas.title) {
      items.push({
	text: this.languages.subtitle,
	iconCls: '',
  ...
  simpleSearch: function() {
  {
    var win = new Ext.Window({
      title: this.languages.searchOnObjects,
      width: 550,
      height: 300,
      x: 50,
      layout: 'anchor',
...
//           height: 35,
          items: [{
            ...
            items: [{
              xtype: 'textfield',
              emptyText: this.languages.acceptedRegex,
              enableKeyEvents: true,
              width: 200,
              listeners: {
                scope: this,
              ...
              xtype: 'checkbox',
              boxLabel: this.languages.advancedOptions,
              checked: false,
              listeners: {
                scope: this,
            ...
            items: [{
              ...
              xtype: 'checkbox',
              boxLabel: this.languages.oneLineByObject,
              checked: true
            }]
          }]
        }, {
          ...
          cm: new Ext.grid.ColumnModel({
              ...
              columns: [
                { header: this.languages.type, dataIndex:'type' },
                { header: this.languages.fieldsearched, dataIndex:'name', width: 120 },
                { header: this.languages.valuefound, dataIndex:'detail', id: 'detail' }
            ]})
        }],
      bbar: {
        items: [
          {
            text: this.languages.stopHighlighting,
            scope: this,
            handler : function(b) {
          ...
          '->',
          {
            text: this.languages.highlightObject,
            scope: this,
            handler : function(b) {
          ...
          {
            text: this.languages.editSelectedObject,
            scope: this,
            handler : function(b) {
              var grid = win.items.items[1], rows = grid.getSelectionModel().getSelections();
              if(rows && rows.length)
  ...
  deleteNode: function(n) {
    if(typeof(n) != 'object' || !n.length) n = [n];
    var str1 = n.length > 1? this.languages.selectedObject + ' ' : this.languages.thisObject + ' ',
        str2 = n.length > 1? this.languages.selectedObjectsAndAllThe : this.languages.thisObjectAndAllIts;
    Ext.Msg.confirm('Attention', this.languages.doYouWantToDelete + ' '+str1+'? '+str2+ this.languages.linksWillBeDeleted + ' ', function(b) {
      if(b == 'yes')
      {
  ...
  constructor: function(config, extCanvas) { 
   if(!config) config = {};
    ...
    var item1 = [
      ...
      {
        xtype: 'compositefield',
        fieldLabel: this.extCanvas.languages.title,
        items: [
          ...
          {
            xtype: 'checkbox',
            checked: isMulti? false : parseInt(config.hideLabel) || config.hideName,
            boxLabel: this.extCanvas.languages.hide + ' ' + this.extCanvas.languages.title + ' ?',
            name: 'hideLabel',
            flex: 1
          }
        ]
      ...
      {
        xtype: 'compositefield',
        fieldLabel: this.extCanvas.languages.tooltip,
        items: [
          { xtype: 'textfield', value: isMulti? null : config.tooltip || config.name || '',
            hideLabel: h.tooltip, hidden: h.tooltip, name:'tooltip', width:80 },
          {
            xtype: 'checkbox',
            checked: isMulti? false : parseInt(config.hideTooltip),
            boxLabel: this.extCanvas.languages.hide + ' ' + this.extCanvas.languages.tooltip + ' ?',
            name: 'hideTooltip',
            flex: 1
          }
        ]
    ...
    var item2 = [
      { xtype: 'colorfield', fieldLabel: this.extCanvas.languages.color, allowBlank: isMulti,
        name:'color', value: isMulti? null : config.color || 'rgb(255,0,0)',
        hideLabel: h.color, hidden: h.color },
      {
        xtype: 'compositefield',
        fieldLabel: this.extCanvas.languages.edge,
        items: [
          ...
          { xtype: 'displayfield', value: this.extCanvas.languages.width+':', width: 37, style:'margin-top:3px' },
      ...
      {
        xtype: 'compositefield',
        fieldLabel: this.extCanvas.languages.pattern,
        items: [
      ...
      {
        xtype: 'compositefield',
        fieldLabel: this.extCanvas.languages.size,
        items: [
          { xtype:'numberfield', name:'Taille', hideLabel: h.size, hidden: h.size,
            width: 30, value: isMulti? null : config.size || '1.0' },
          { xtype: 'displayfield', value: this.extCanvas.languages.width+':', width: 35, style:'margin-top:3px' },
          { xtype:'numberfield', name:'width', hideLabel: h.width, hidden: h.width,
            width: 30, value: isMulti? null : config.width || '' },
          { xtype: 'displayfield', value: this.extCanvas.languages.height+':', width: 40, style:'margin-top:3px' },
      ...
      {
        xtype: 'combo',
        fieldLabel: this.extCanvas.languages.objectType,
        allowBlank: isMulti,
        hideLabel: h.shape, hidden: h.shape,
        emptyText: this.extCanvas.languages.requiredSelection,
        triggerAction: 'all',
      ...
      { fieldLabel: this.extCanvas.languages.imagePath, value: isMulti? null : config.imagePath || '',
        ...
        allowBlank: true, name:'imagePath', disabled: config.shape != 'image' },
      {
        ...
        boxLabel: this.extCanvas.languages.theObjectIsFixed,
        name: 'eventless',
        flex: 1
      }
    ];
    var ps = [{
      title: this.extCanvas.languages.properties,
      items: item1
    }];
    ...
    if(config.id)
      bitems.push({
        text: this.extCanvas.languages.cancel,
        scope: this,
        handler: function(b, e) { Ext.canvasXpress.utils.resetForms(b.ownerCt.ownerCt.get(0)) }
      },
      '|');
    bitems.push({
        text: this.extCanvas.languages.apply,
        handler: function(b, e) { bf(b, e, true) }
      },
      '|',
      {
        text: isMulti? this.extCanvas.languages.edit + ' ' + this.extCanvas.languages.objects : (config.id? this.extCanvas.languages.edit : this.extCanvas.languages.add) + ' ' + this.extCanvas.languages.object,
        handler: bf
      });
    Ext.apply(this, {
      width: nd.width || 400,
      height: nd.height || 400,
      title: this.extCanvas.languages.objectEditor + (config.id? ' ('+this.extCanvas.languages.objectId+':' + config.id + ')':''),
      items: {
        xtype: 'tabpanel',
        activeItem: 0,
        defaultType: 'form',
      ...
      items: [
        {
          ...
          store: new Ext.data.ArrayStore({
            ...
            data: allNodes }),
          valueField: 'id',
          displayField: this.extCanvas.languages.title,
          tpl: '<tpl for="."><div ext:qtip="Id:{id}" class="x-combo-list-item">{label}</div></tpl>',
    ...
    var ps = [{
      ...
      items: [
        ...
        },
        {
          xtype: 'checkbox',
          name: 'shared',
          hideLabel: parseInt(config.hideLabel),
